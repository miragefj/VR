...
CBitmapFont::CBitmapFont(HDC dc, int screen_width, int screen_height, const char *font, int size)
{
	Init(dc, screen_width, screen_height, font, size, size);
}
...
int CBitmapFont::Init(HDC dc, int w, int h, const char *fontname, int size, int maxchars)
{
	...
	fontListID = glGenLists(m_nMaxChars);					// Generate the list for the font
	...
	hFont = CreateFont(size, // Our desired HEIGHT of the font
		...
		fontname);					// The font name (Like "Arial", "Courier", etc...)
...
// We store the old font so we can select it back in when we are done to avoid memory leaks.
	m_hOldFont = (HFONT)SelectObject(m_hDev, hFont);
	...
	wglUseFontBitmaps(m_hDev, 0, m_nMaxChars - 1, fontListID);	// Builds 255 bitmap characters
	return 1;
}

void CBitmapFont::Shutdown()
{
	glDeleteLists(m_FontListId, m_nMaxChars);				// Free the display list
	SelectObject(m_hDev, m_hOldFont);						// Select the old font back in so we don't have memory leaks
}

void CBitmapFont::Text(int x, int y, const char *strString, ...)
{
	...
	glListBase(m_FontListId);							// This sets the lists base
...
void CBitmapFont::PositionText(int x, int y)
{
	...
	y = m_nHeight - m_nFontHeight - y;				// Calculate the weird screen position
